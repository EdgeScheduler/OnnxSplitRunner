...
def run():
    ...
    session2 = onnxruntime.InferenceSession('Onnxs/vgg19/childs/0/vgg19-0.onnx', providers=['CUDAExecutionProvider']) 
    # session1 = onnxruntime.InferenceSession('Onnxs/vgg19/childs/1/vgg19-1.onnx', providers=['CUDAExecutionProvider']) 
    ...
    # session_ = onnxruntime.InferenceSession('Onnxs/vgg19/vgg19.onnx', providers=['CUDAExecutionProvider']) 


    if session0 is not None and session1 is not None and session2 is not None and session0 is not None:
        print("end")

...
def run_by_routine(processes,batch=data_batch):
    async def run(processes,batch):
        ...
        process_start=time.time()
        for idx in range(len(model_names)):
            myroutines.append(asyncio.create_task(signal_routine(model_names[idx],sessions[idx], labels[idx], datas[idx])))
        ...
        print()
        return
    asyncio.run(run(processes,batch))
